...
Finally, even if you use a custom encrypted DNS server (DoS or DoT) with eSNI support, it might still not be enough as traffic analysis studies[^29] have shown it's still possible to reliably block unwanted requests. Only the Tor network was able to demonstrate efficient DNS Privacy.

Therefore to mitigate this issue, this guide we will later recommend a solution of VPN over Tor to benefit from the best possible protection.
...
There are specialized commercial forensics solutions available[^92] that will help an adversary analyze your cloud data with ease.
...
The digital fingerprint is the way you write, the way you behave. The way you click. The way you browse. The fonts you use on your browser[^93]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
A real use and well-documented case of this was the arrest of the hacker Jeremy Hammond[^94] who shared over time several details about his past.
...
Your Browser and Device Fingerprints^137^ are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance websites will be able to provide a "mobile experience" if you're using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^95] based browsers (such as Chrome) or Firefox[^96] unless taking special measures.
...
Even if you're not looking at the camera, they can still figure out who you are[^102] and even make out your emotions[^103].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^104][^105][^106][^107][^108].
...
We can safely imagine a near future where you won't be able to create accounts or sign-in anywhere without providing unique biometrics (A good time to re-watch Gattaca[^111], Person of Interest[^112] and Minotiry Report[^113]).

Phishing:
---------

Phishing[^114] is a type of attack where an adversary could try to extract information from you by pretending to be something/someone else.
...
TAILS[^116] stands for **The Amnesic Incognito Live System.** . It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
Ideally (but not mandatory[^117]), you should get a dedicated that won't be tied to you in any easy way laptop (paid with cash anonymously as previously mentioned for the phone and the SIM card).
...
-   Enable BIOS/UEFI password and use a long passphrase[^118] instead of a password if you can.
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^119] (with pictures).
...
This route will make extensive use of Virtual Machines[^120], they'll require a host OS to run the Virtualization software. You have two choices in this part of the guide:

-   Your Linux distribution of choice (excluding Qubes)

-   Windows 10 Home edition

Linux is not necessarily the better choice for privacy depending on your threat model. This is because using Windows will allow us to use a privacy feature called Plausible Deniability[^121].
...
A (nice) adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not not incriminate yourself") but some countries are implementing laws[^122] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
This guide will recommend using Tor browser within the host OS because it has the best protections by default.

This Browser will only be used to download various utilities and will never be used for anonymous activities.
...
Veracrypt[^124] is the software we will use for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
The second catch is the "Cold Boot attack"[^127] discussed briefly earlier in this guide. This is trickier than the Evil Maid Attack as it requires them to come into possession of your laptop while you're actively using the hidden OS/data or very shortly afterward.

The principle is simple, as shown in this video[^128], they could theoretically quickly boot your laptop on a special USB key that would copy the content of the RAM (the memory) of the laptop after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance). The memory won't decay and they could be able to copy its content for analysis. This memory dump should contain the key to decrypt your hard drive/data. To mitigate this you have to apply a few principles:

-   Encrypt the memory with a Veracrypt option[^129] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Eventlogs[^130].
...
There have been some forensics studies[^131] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^132] and by the maintainer of Veracrypt[^133].
...
A very good example of this are the Windows "ShellBags" that are stored within the Windows Registry storing various histories of accessed files/folders[^134].
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim Operations[^135]. This will severely degrade the performance/health of your SSD drive over time.
...
-   To rescue disk[^141] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
In the Whonix route, we will make extensive use of the free Oracle Virtualbox[^142] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like KVM in Linux, feel free to do so).
...
-   DNS Leaks to your ISP/Google are impossible because the VM is on an isolated network that has to go through Tor no matter what.
...
This other illustration shows the other possibility without a cash-paid VPN. It's very similar but has one less defense layer. If the Tor network is compromised by a Global/State actor, then you will be de-anonymized immediately.
...
For these reasons, I strongly recommend the addition of a cash-paid VPN to the mix.
...
This route will use Virtualization and Whonix[^143] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
-   Performance is better than Tor Browser

If you want to use Tor Browser, skip these step, if you want to follow my recommendation, follow these steps:
...
You will need something to store your data (logins/passwords, identities and TOTP[^144] information).

For this purpose I strongly recommend KeepassXC because of their TOTP feature. This is the ability to create entries for 2FA[^145] authentication with the authenticator feature.
...
Captcha[^146] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^147] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^148]) and Cloudflare (hCaptcha[^149]). hCaptcha is used on 15% of the internet by their own metrics[^150].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^151]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^152].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^153].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/Protonmail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^154].
...
Browser and Device[^155] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^156]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^157] and unfortunately sometimes required.
...
This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^158].
...
-   Origin IP geolocation (your IP/location when you go to "whatsmyipaddress.com") should match your identity location as much as possible (You can pick this in the VPN client if you use the 3 layers approach or just create a new identity in Tor Browser or Brave until you get the appropriate Exit node). You should exclude any exit IP that is not located in Western Europe/North America/Japan/South Korea/Australia (excluding Mexico) as you will have less issues with those. Ideally, you should get an European Union IP to get additional GDPR protection and if possible a German exit IP due to their legal stance on using anonymous accounts on online platforms.

-   Brave Browser with a Private Tor Tab has a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms^137^ if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
They're recommended by privacytools.io[^159]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^160]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
If your account gets suspended [^163] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^164] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
This time you will need to submit an appeal using a form [^165] , provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on Protonmail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your Protonmail.
...
Linkedin will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^166] that specializes in ID proofind. This is most likely a dead end as this would force you to develop some strong photoshop skills.
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^167] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
-   Use an Android Emulator (Youwave[^168], Bluestacks[^169] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   Use the Wizard:
        ...
        -   Select Clean Options **(be careful with the free space one if you are running the decoy OS. Do not erase the free space or anything else on the second partition)**
...
There are several utilities that are recommend (like the old unmaintained DBAN[^170]) for this but personally, I will recommend the use of ShredOS.
...
As written earlier in this guide and as noted by Privacytools.io[^171], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
[^93]: EFF Panopticlick (<https://panopticlick.eff.org/>)

[^94]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>

[^95]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^96]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^97]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^98]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^99]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>
...
[^101]: Brown Institute, Open Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^102]: NewScientist, Facebook can recognise you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^103]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^104]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^105]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^106]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^107]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^108]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^109]: Anyvision <https://www.anyvision.co/>

[^110]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^111]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^112]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^113]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^114]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^115]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^116]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^117]: It's not mandatory because this guide will help you harden your laptop as much as possible to prevent online leaks through various means. There will be several lines of defense standing between your online identities and yourself that should prevent most adversaries from de-anonymizing you besides state/global actors with considerable resources.

[^118]: XKCD, Password Strength <https://xkcd.com/936/>

[^119]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^120]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^121]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^122]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^123]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^124]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^125]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^126]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^127]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^128]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^129]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^130]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^131]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^132]: SANS.org, Mission Implausible:Defeating PlausibleDeniability with DigitalForensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^133]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^134]: SANS, Windows ShellBag Forensics inDepth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^135]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^136]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^137]: WindowsCentrel, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^138]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^139]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^140]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^141]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^142]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^143]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^144]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^145]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^146]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^147]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^148]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^149]: hCaptcha <https://www.hcaptcha.com/>

[^150]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^151]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^152]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^153]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>
...
[^155]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^156]: Developers Google Blog,
...
[^157]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^158]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^159]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^160]: Protonmail Human Verification System <https://protonmail.com/support/knowledge-base/human-verification/>

[^161]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^162]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^163]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^164]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^165]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^166]: Jumio, ID verification features <https://www.jumio.com/features/>

[^167]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^168]: Youwave, <https://youwave.com/>

[^169]: Bluestacks, <https://www.bluestacks.com/>

[^170]: DBAN, <https://dban.org/>

[^171]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>
